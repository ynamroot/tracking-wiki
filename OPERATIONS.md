...
7. 상충하는 주장을 `모순` 절과 `open-questions.md`에 기록한다.
8. 변경된 모든 페이지의 `updated`, `sources`, `confidence`를 갱신한다.
9. 문장 작성 원칙 자기 점검: `SCHEMA.md`의 문장 작성 원칙대로 약어 첫 등장 풀이, 용어 통일, 한영 혼용 제거, `관련 문서`의 [[key-terms|핵심 용어 해설]] 링크, claim 옆 앵커를 확인하고, 비전문가가 본문만 읽고 이해할 수 있는지 검토한다.
10. `wiki/index.md`에 새 페이지와 한 줄 설명을 추가한다.
11. `wiki/log.md`에 `ingest` 기록을 append한다.
12. 검증 스크립트를 실행하고 Issue를 `ingest:complete`로 전환한다.
...
- Source가 빈약하면 Wiki 본문도 확정적으로 부풀리지 않고 `confidence: low`, `미결 질문`, `needs-verification`을 명시한다.
- 비전문가(데이터매니저, 기획자, 프론트엔드 개발자)도 본문만 읽고 이해할 수 있게 쓴다. 한 문장 안에서 한국어와 영어를 섞지 않는다.
- 외래어와 약어는 각 문서에서 첫 등장 시 쉬운 한국어로 풀고, 모든 지식 페이지와 자료 요약은 `관련 문서`에 [[key-terms|핵심 용어 해설]]을 연결한다.

### Page Splitting And Knowledge Taxonomy

`knowledge` 하위의 현재 4개 축은 탐색을 위한 상위 분류다. 세부 지식은 폴더보다 페이지 단위로 분할하고, index와 `관련 문서`로 방향을 잃지 않게 연결한다.
...
없는 핵심 개념, 조사 공백, 가독성 위반(한영 혼용, 미해설 약어,
영어 절 이름, 앵커 몰림, key-terms 링크 누락)을 찾고 audit report를 작성하라.
