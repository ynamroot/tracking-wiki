...
1단계는 항상 공개한다. 2~4단계 산출물은 PO가 공개를 결정하기 전까지 비공개로 다룬다. 자세한 원칙은 아래 "비공개 제품 초안"을 참고한다.

## 비공개 제품 초안

제품 아이디어 원문과 그 판정 보고서는 이 공개 저장소 밖 비공개 작업 공간에 둔다. 이 저장소에는 검증 과정에서 나온 일반화된 조사 질문, 조사 공백, 검토 기록만 남긴다. 문서 우선순위 관점에서 비공개 초안은 이 저장소의 근거가 아니라 Wiki 근거로 검증받는 대상이다. 절차는 `OPERATIONS.md`의 "제품 초안 논증" 절을 따른다.

같은 원칙이 PRD, Feature Spec, Technical Spec, ADR에도 이어진다. 이들은 흩어진 Wiki 사실을 타깃 고객·요구사항·성공 지표·구현 방식 하나로 압축한 실행 청사진이라, 사실 하나하나가 이미 공개돼 있어도 그 종합 자체가 더 구체적인 노출이다. 그래서 생명주기 2~4단계 산출물도 PO가 `PD-NNN`으로 공개 수위를 결정하기 전까지 같은 비공개 작업 공간에 머문다. 결정 후 공개된 범위만 `product/drafts`, `technical/drafts`에 들어오고, 이후 승인 절차(`OPERATIONS.md` 8~9절)를 따른다.
...
- 비공개 제품 초안, PRD, Feature Spec, Technical Spec의 내용은 PO가 공개를 결정하기 전까지 어떤 형태로도 이 저장소에 기록하지 않는다.
