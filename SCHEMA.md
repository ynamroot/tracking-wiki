...
상위 폴더만으로 주제를 충분히 설명하려고 하지 않는다. 세부성은 파일 경로보다 페이지 제목, H1, `현재 종합`, `관련 문서`, `wiki/index.md`의 한 줄 설명으로 드러낸다. 필요한 경우 같은 상위 폴더 안에 좁은 주제의 페이지를 추가한다.
...
`status`는 기계가 읽는 값이라 영어를 유지한다. 다만 `current`를 기본값으로 방치하지 않는다. `모순` 절에 페이지의 핵심 결론을 흔드는 미해결 충돌이 있으면 `disputed`, 근거 Source가 갱신 없이 낡았거나 시장 사실이 바뀌었을 가능성이 높으면 `stale`, 새 페이지가 대체하면 `superseded`로 바꾸고 `wiki/log.md`에 남긴다.

필수 본문(절 이름은 한국어로 쓴다):

- `현재 종합` (구 Current Synthesis)
- `근거` (구 Evidence)
- `모순` (구 Contradictions)
- `미결 질문` (구 Open Questions)
- `제품 시사점` (구 Product Implications)
- `관련 문서` (구 See Also)

권장 본문:

- `배경` (구 Context): 독자가 이 페이지를 읽어야 하는 이유와 범위.
- `작동 방식` 또는 `업무 흐름` (구 Mechanics / Workflow): 기능, 기술, 시장 구조가 실제로 어떻게 작동하는지.
- `평가 기준` (구 Evaluation Criteria): 비교, benchmark, 기술 가설 검증에 사용할 기준.
- `한계` (구 Limits): Source coverage, 적용 불가능한 상황, 알려진 약점.

문서가 짧아도 필수 절은 유지한다. 단, 내용이 없으면 빈칸으로 두지 않고 "현재 accepted Source로는 확인되지 않았다"처럼 불확실성을 명시한다.

핵심 논지(예: 화면 행동 → 기대 이벤트 → 실제 전송 내용 사이의 증거 공백)는 단 하나의 canonical 페이지가 전체 서술을 소유한다. 다른 페이지는 한 문장 요약과 canonical 페이지 wikilink만 둔다. `overview`는 진입 문서로서 한 문단 요약까지 허용한다.

문장 작성 원칙:

- 이 Wiki의 독자는 두 층위다. 비전문가도 본문만 읽고 이해할 수 있어야 하고, 전문가는 같은 문서에서 판단 근거(`근거`·`모순`·`출처`)를 얻을 수 있어야 한다. 쉬운 한국어가 기본 문체이고, 원어와 약어는 보조다. 한 문장 안에서 한국어와 영어를 섞어(예: "release와 analytics 사이의 책임 공백") 쓰지 않는다.
...
- 영어 약어는 독자가 바로 알 수 없다고 가정한다. `SDK`, `CDP`, `CI`, `PR`처럼 자주 쓰는 약어도 각 문서에서 첫 등장 시 풀어 쓴다. 예: `고객데이터플랫폼(CDP)`.
- 문단은 한 가지 주장만 다룬다. 한 문단에 시장 설명, 기술 설명, 제품 제안이 섞이면 절을 나눈다.
- 표는 비교에만 사용하고, 표 아래에 “그래서 무엇을 뜻하는가”를 한 문단으로 설명한다.
- 모든 지식 페이지와 자료 요약은 `관련 문서`에 [[key-terms|핵심 용어 해설]]을 연결한다. `key-terms` 자신만 예외다.
...
- 출처 앵커는 문단 전체가 아니라 구체 claim에 가깝게 붙인다. `현재 종합` 문단도 예외가 아니다. 문단 끝에 앵커를 몰아 붙이지 말고, 각 claim 문장 바로 뒤에 해당 Source의 앵커를 붙인다. 가격, 수치, 기능 지원 여부, 고객 사례, benchmark 구성, 날짜가 있는 주장은 반드시 앵커를 붙인다.
- Source summary에만 근거가 있다는 이유로 지식 페이지의 출처 표기를 생략하지 않는다. 지식 페이지 본문도 독립적으로 claim을 추적할 수 있어야 한다.

여러 Source에서 도출한 해석은 `현재 종합`이라고 명시하고, 제품 제안은 `제품 시사점`으로 분리한다.

## Open Question

`wiki/open-questions.md`가 미결 질문의 단일 목록이다. 각 지식 페이지의 `미결 질문` 절은 이 단일 목록을 참조하는 지역 인덱스다.
...
- 지식 페이지의 `미결 질문` 절에는 질문 본문만 반복하지 말고, `[[open-questions|OQ-NNN]]` 또는 `OQ-NNN`을 명시해 중앙 목록과 연결한다.
- 해소된 질문은 삭제하지 않는다. `해소 기록` 또는 해당 항목의 설명에 근거 Source와 반영 문서를 남긴다.
