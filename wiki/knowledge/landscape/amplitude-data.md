...
- Ampli는 타입 안전 API와 CI status 검사를 제공한다. <sup>[🔗](#source-1)</sup>
- Observe는 유입 schema의 current, invalid, out-of-date 상태를 구분한다. <sup>[🔗](#source-1)</sup>
- 코드 계측에서는 개발자가 정확한 호출 위치를 선택해야 한다. <sup>[🔗](#source-1)</sup>
- UI 요소 매핑은 별도 Autocapture와 Visual Labeling 영역이다. <sup>[🔗](#source-1)</sup>
