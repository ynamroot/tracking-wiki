...
- Snowplow와 dbt 관련 Source는 실패 기록(failure artifact)과 계약 강제(contract enforcement)의 안전 모델(safety model)을 참고하게 해준다. <sup>[🔗](#source-5)</sup>
- Heap, PostHog, Amplitude, GTM 공식 문서는 이름표(label), 액션(action), 트리거(trigger)가 선택자나 DOM 조건에 기대는 경우가 있음을 보여준다. 선택자는 빠르게 시작하기 쉽지만 화면 구조가 바뀌면 같은 요소를 안정적으로 찾기 어렵다. <sup>[🔗](#source-7)</sup>
...
상용 도구들은 보통 증거의 한 조각만 남긴다. 태그 감사 도구는 네트워크 요청과 규칙 위반을 잘 남기고, 오토캡처 도구는 원시 클릭과 사후에 붙인 이름표(label)를 남기며, 태그매니저는 트리거(trigger)와 태그 발화(tag firing)를 보여준다. 이 프로젝트의 증거 모델은 이 조각들을 한 화면 행동 단위로 묶는 것을 목표로 해야 한다.

## 평가 기준

- 안정성: 화면 구조, 문구, CSS 클래스(class), 배치(layout)가 바뀌어도 같은 요소를 찾는가.
...
초기 제품은 자동 수정보다 증거를 먼저 보여주는(proof-first) 방식으로 시작해야 한다. 사람에게 “이 요소가 이 이벤트를 내야 하는데 이번 빌드(build)에서 안 나왔다”를 재현 가능하게 보여주는 것이 우선이다.

## 관련 문서

- [[autonomous-traversal-evaluation|자율 순회 평가 기준]] - 증거 수집 자동화
- [[schema-and-data-contracts|스키마 및 데이터 계약]] - 하류 계약 연결
