...
- ObservePoint 공식 문서는 Audit이 브라우저에서 페이지를 방문해 태그 요청을 지켜보고, Journey가 폼과 퍼널(funnel) 같은 핵심 경로를 흉내 내 실행한다고 설명한다. 이는 자동 검증이 이미 존재한다는 근거지만, 사람이 정해 둔 범위와 규칙에 의존한다. <sup>[🔗](#source-2)</sup>
- Trackingplan 공식 문서는 실사용자 트래픽에서 이벤트·속성·픽셀(pixel)·UTM·dataLayer·동의 신호(consent signal)를 찾아내 살아 있는 트래킹 플랜(tracking plan)을 만든다고 설명한다. 이는 실제 트래픽 관찰에 강하지만, 아직 발생하지 않았거나 계측이 빠진 화면 행동은 직접 만들어 내지 않는다. <sup>[🔗](#source-2)</sup>
- Heap, PostHog, Amplitude, Mixpanel 공식 문서는 오토캡처가 원시 상호작용(raw interaction)을 먼저 모으고 라벨·행동을 나중에 정의하는 방식임을 확인한다. 다만 DOM(브라우저가 화면을 나무 구조로 표현한 것) 변경, 선택자(selector) 조건, 민감정보 기본 제외, 수동 복구 같은 운영 부담이 남는다. <sup>[🔗](#source-2)</sup>
- Google Tag Manager 공식 문서는 요소 표시 트리거(trigger)가 ID나 CSS 선택자를 쓰고, 미리보기·디버그(preview/debug)가 사람이 직접 탐색하는 세션 기반 검증 흐름임을 보여 준다. <sup>[🔗](#source-2)</sup>
...
제출 문서(MD)의 DataTrue 가격과 일부 제품별(vendor) 세부 한계는 공식 문서로 다시 확인하지 못했다. 현재 Wiki에는 시장 분류와 공백 가설에만 반영하고, 가격·등급(tier)·정량 주장(claim)에는 쓰지 않는다.

## 미결 질문

- `OQ-002`: 다섯 갈래 중 어느 계층의 예산을 먼저 가져올 수 있는지에 따라 첫 최소기능제품(MVP)의 성과가 달라진다.
- `OQ-005`: 선택자보다 안정적인 요소 지문이 실제 화면 개편에서 얼마나 잘 유지되는지 검증해야 한다.
- `OQ-007`: 공식 문서 보강 범위는 넓어졌지만, 가격과 엔터프라이즈 전용(enterprise-only) 기능은 의사결정 직전 다시 확인해야 한다.

## 제품 시사점

제품의 위치는 “또 하나의 분석 도구”가 아니라 “기존 분석·거버넌스 도구의 앞단에서 화면 행동과 이벤트 전송을 증명하는 검증 계층”이어야 한다. 특히 오토캡처와 태그 감사 도구가 이미 존재하므로, 차별화는 전체 사이트를 많이 도는 능력이 아니라 바뀐 화면, 기대 이벤트, 실제 전송 내용, 사람이 확인할 수 있는 증거를 하나로 묶는 데 있다.
