...
- Ampli는 플랜에서 타입 안전 wrapper를 생성해 필수 속성과 enum 오류를 컴파일 또는 런타임에 잡는다. [SRC-20260721-amplitude-data-ampli]
- `ampli status`는 코드의 Ampli 호출과 플랜을 대조해 미구현 이벤트를 CI에서 표시한다. [SRC-20260721-amplitude-data-ampli]
- Observe는 유입 이벤트를 Unexpected, Valid, Invalid, Out of Date로 분류하고 경고·차단·변환을 지원한다. [SRC-20260721-amplitude-data-ampli]
- 생성된 함수를 코드의 정확한 위치에서 호출하고 검증 경고를 운영하는 일은 사람에게 남는다. [SRC-20260721-amplitude-data-ampli]
- Ampli 자체에는 UI 요소와 실제 이벤트 발화의 시각적 인과 증빙이 없다. [SRC-20260721-amplitude-data-ampli]
